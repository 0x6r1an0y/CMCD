...
uint8_t Adafruit_PN532::mifareclassic_WriteDataBlock(uint8_t blockNumber,
  ...
  // if write successfully the byte 7-8 should be 0x00 0xEA
  // if write failed the byte 7-8 should be 0x01 0xE9

  if (pn532_packetbuffer[7] == 0x01 || pn532_packetbuffer[8] == 0xE9){
    return 0;
  }
...
/*
PN532 reboot 
return true -> reboot successfully
return true -> reboot failed
In order to make PN532 detect magic backdoor command continuously, I have to make this function.
delay 50 millisecond
*/
bool Adafruit_PN532::reboot() {

  uint8_t cmdState1[3] = {PN532_COMMAND_POWERDOWN, 0x00, 0x00};
  if (!sendCommandCheckAck(cmdState1, 3)) {
  #ifdef PN532DEBUG
        PN532DEBUGPRINT.print(F("failed to power down."));
  #endif
    return false;
  }
  delay(10);
  begin();
  /*
  uint8_t cmdState2[3] = {PN532_WAKEUP, 0x00, 0x00};
  if (!sendCommandCheckAck(cmdState2, 3)) {
  #ifdef PN532DEBUG
        PN532DEBUGPRINT.print(F("failed to wake up."));
  #endif
    return false;
  }
  */
